		...
		$.post("prueba.php" ,{ name:user})
			.done(function(data){
			 if(data.length>0){
	            ...
	            $("#usuario").css("background-color","#F96965");  
		 ...
		 $.post("pruebaemail.php" ,{ campo:email})
			.done(function(data){
			 if(data.length>0){
	            
	            $("#email").css("background-color","#F96965");  
...
/*         $("#validar").click(function(){
		  ...
		  var pass= $("#pass").css('backgroundColor');
		  var usuario= $("#usuario").css('backgroundColor');
		  var email= $("#email").css('backgroundColor');
		  var color= "rgb(249, 105, 101)";
  
  
		  if(pass== color || usuario==color || email==color){
		  
				alert('Introduzca todos los campos');
				document.write("<html><head></head><body onload=\"javascript:history.back(1)\"></body></html>");
		  }else{
			
			 $('#form').attr('action', 'registrar.php');
...
<img alt="full screen background image" src="img/descarga.jpg" id="imagen" />
    ...
    <input class="circ2" type="radio" name="radio" value="Mujer"/>
	 ...
	 <input   type="text"  action="registrar.php"  name="telefono">
