...
require_once('funcedad.php');

$edadactual= busca_edad($nacimiento);

$dato= explode("-", $nacimiento);  
$actanno= $dato[0]; 
$ano=date("Y");
$difano= $ano-$actanno;
    ...
    if($email==$remail){
		...
		if($filas>0){
			...
			<body onload=\"javascript:history.back()\">
			 ...
			 if($edadactual<18 || $difano>130){
	                    
						if($edadactual<18 ){
							echo "<script> alert('No se permiten registros a menores de edad')</script>";	
							echo "<html><head></head>
							<body onload=\"javascript:history.back()\">
							</body></html>";
						}else{
							
							echo "<script> alert('Debe introducir una fecha de nacimiento valida')</script>";	
							echo "<html><head></head>
							<body onload=\"javascript:history.back()\">
							</body></html>";
						};
		    ...
		    if($pass !=$repass){
				...
				echo "<script> alert('La contraseña tiene que tener mínimo 6 caracteres')</script>";	
				echo "<html><head></head>
				<body onload=\"javascript:history.back()\">
				</body></html>";
			};
